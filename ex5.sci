...
function [yn, yorigin] = multi(x1n, x1origin, x2n, x2origin)
    ...
    // Perform multiplication by iterating through the common time axis
    for i = 1:length(n_y)
        t = n_y(i);
        val1 = 0;
        val2 = 0;
        ...
        yn(i) = val1 * val2; // Multiplication operation
    end
    ...
    xtitle("Multiplication signal y(n) = x1(n) * x2(n)", "n", "y(n)");
    xgrid();
    a = gca();
    a.data_bounds = [x_min, y_min_all; x_max, y_max_all];
endfunction

// Test using the same inputs as Exercise 4
[yn, yorigin] = multi([0, 1, 3, -2], 1, [1, 1, 2, 3], 2);
yn      // expected answer: [0, 0, 2, 9, 0]
